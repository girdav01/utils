...
suffix = "&adversary=turla"  #each json file on mitre-engenuity.org got this
filesuffix = '_turla.json'

#list of vendors in this year eval
#can extract with:
#   curl -sk 'https://attackevals.mitre-engenuity.org/api/participants/' | jq '.[] | select(.adversaries_completed[].name == "turla") | .name'
vendors = ["ahnlab","bitdefender","blackberry","crowdstrike","cybereason","cynet","deepinstinct","elastic","eset","fortinet","harfanglab","ibmsecurity","malwarebytes","microsoft","paloaltonetworks","qualys","rapid7","secureworks","sentinelone","somma","sophos","symantec","tehtris","trellix","trendmicro","uptycs","vmware","watchguard","withsecure"]
